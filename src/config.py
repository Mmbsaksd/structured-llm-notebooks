...
API providers so students are not tied to a paid OpenAI key:
    ...
    LLM_PROVIDER=openai|anthropic|gemini|groq|azureopenai
...
# Provider selection
# ---------------------------------------------------------------------------
VALID_PROVIDERS = ("openai", "anthropic", "gemini", "groq", "azureopenai")
...
AZURE_OPENAI_ENDPOINT: str | None = os.getenv(
    "AZURE_OPENAI_ENDPOINT") or os.getenv("AZURE_OPENAI_API_BASE")
...
def get_model(provider: str | None = None) -> str:
    ...
    if provider == "azureopenai":
        return "gpt-4o-mini" if USE_SMALL_MODEL else AZURE_OPENAI_MODEL
...
def get_azureopenai_model() -> str:
    """Return the Azure OpenAI model name based on cost-saving toggles."""
    return get_model("azureopenai")
...
def get_outlines_model(provider: str | None = None):
    ...
    if provider == "azureopenai":
        ...
        return outlines.from_openai(client, get_model("azureopenai"))
...
def get_azureopenai_client():
    """Return a plain Azure OpenAI client."""
...
def get_async_azureopenai_client():
    """Return a plain async Azure OpenAI client."""
